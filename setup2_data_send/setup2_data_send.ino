...
//const char* ssid     = "Senz Cloud";
//const char* password = "GS20010423";
...
const int ANALOG_PIN     = 27; // Reads the analog soil moisture (AO)
const int DIGITAL_PIN    = 26; // Reads the digital threshold (DO)

// Onboard LED pin (GPIO 2 on most ESP32 boards)
// LED ON = WiFi connected | LED OFF = WiFi disconnected
...
void setup() {
  ...
  digitalWrite(ONBOARD_LED_PIN, LOW); // Start with LED OFF while connecting
  ...
  digitalWrite(ONBOARD_LED_PIN, HIGH); // LED ON — WiFi connected
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    digitalWrite(ONBOARD_LED_PIN, LOW);  // LED OFF — WiFi lost
  }
